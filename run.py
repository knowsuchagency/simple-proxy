#!/usr/bin/env python
"""Task runner for the simple-proxy project."""
...
def main():
    """
    Development tasks for simple-proxy
...
def autopep8(auto_commit):
    ...
    shell('autopep8 -i -r simple_proxy/ tests/')
...
def clean_build():
    ...
    click.confirm('This will uninstall the simple_proxy cli. '
...
def test(capture, pdb, mypy):
    ...
    if mypy:
        shell('mypy simple_proxy tests/ --ignore-missing-imports')
...
def coverage(no_browser):
    ...
    shell('coverage run --source simple_proxy -m pytest')
...
def docs(no_browser):
    ...
    """
    shell('rm -f docs/simple_proxy.rst')
    shell('rm -f docs/modules.rst')
    shell('rm -f docs/simple_proxy*')
    shell('sphinx-apidoc -o docs/ simple_proxy')
